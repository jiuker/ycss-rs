<template>
    <div class="f-c fs-1 bc-ff1115 d-f">1231231</div>
    <div class="bc-000-112-231 br-nr bp-c bs-c bs-10-15"></div>
    <div class="b-3-001 br-1-123 o-1-000121 c-fff ls-12 lh-20"></div>
    <div class="ta-c ta-r ta-l"></div>
    <div class="fs-20 fw-100"></div>
    <div class="m-1010 p-0505 h-10 w-21 h10 w10"></div>
    <div class="maxh-23 maxw-10 minh-11 minw-22"></div>
    <div class="p-f p-a p-r d-b t-2 b-1 l-3 r-11 va-m zi-205"></div>
    <div class="mt-10 ml-10 mr-10 mb-10"></div>
    <div class="pt-10 pl-10 pr-10 pb-12 br-4"></div>
    <div class="d-f fd-r ai-c jc-c ai-c fw-nw f-50 test1"></div>
</template>

<style>
    .test{
        width: 10px;
    }
    /* Automatic generation Start */
.f-c{
 display: -webkit-flex;
    display: flex;justify-content:center;align-items:center;
}
.fs-1{
 font-size:2upx;
}
.bc-ff1115{
 background-color:#ff1115;
}
.d-f{
 display: -webkit-flex;
    display: flex;
}
.bc-000-112-231{
 background-color:rgb(000,112,231);
}
.br-nr{
 background-repeat:no-repeat;
}
.bp-c{
 background-position:center;
}
.bs-c{
 background-size:cover;
}
.bs-10-15{
 background-size:20upx 30upx;
}
.b-3-001{
 border:6upx solid #001;
}
.br-1-123{
 border-right:2upx solid #123;
}
.o-1-000121{
 outline:#000121 dotted 2upx;
}
.c-fff{
 color:#fff;
}
.ls-12{
 letter-spacing:24upx;
}
.lh-20{
 line-height:40upx;
}
.ta-c{
 text-align:center;
}
.ta-r{
 text-align:right;
}
.ta-l{
 text-align:left;
}
.fs-20{
 font-size:40upx;
}
.fw-100{
 font-weight:100;
}
.m-1010{
 margin:20upx 20upx;
}
.p-0505{
 padding:10upx 10upx;
}
.h-10{
 height:20upx;
}
.w-21{
 width:42upx;
}
.h10{
 height:10%;
}
.w10{
 width:10%;
}
.maxh-23{
 max-height:46upx;
}
.maxw-10{
 max-width:20upx;
}
.minh-11{
 min-height:22upx;
}
.minw-22{
 min-width:44upx;
}
.p-f{
 position:fixed;
}
.p-a{
 position:absolute;
}
.p-r{
 position:relative;
}
.d-b{
 display:block;
}
.t-2{
 top:4upx;
}
.b-1{
 bottom:2upx;
}
.l-3{
 left:6upx;
}
.r-11{
 right:22upx;
}
.va-m{
 vertical-align:middle;
}
.zi-205{
 z-index:205;
}
.mt-10{
 margin-top:20upx;
}
.ml-10{
 margin-left:20upx;
}
.mr-10{
 margin-right:20upx;
}
.mb-10{
 margin-bottom:20upx;
}
.pt-10{
 padding-top:20upx;
}
.pl-10{
 padding-left:20upx;
}
.pr-10{
 padding-right:20upx;
}
.pb-12{
 padding-bottom:24upx;
}
.br-4{
 border-radius:8upx;
}
.d-f{
 display: -webkit-flex;
    display: flex;
}
.fd-r{
 flex-direction:row;
}
.ai-c{
 align-items:center;
}
.jc-c{
 justify-content:center;
}
.ai-c{
 align-items:center;
}
.fw-nw{
 flex-wrap:nowrap;
}
.f-50{
 -webkit-flex:50;
   flex:50;
}

/* Automatic generation End */
</style>